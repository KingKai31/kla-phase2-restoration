"""
Task 6: Stage B - fine-tune from checkpoints/stage_a_best.pt, adding Task
C's 8,526 synthetic external pairs (train-only, scripts/generate_synthetic_pairs.py)
to the real leakage-checked training set. Base 5-term StageBCompositeLoss
only - the ROI-preservation term is excluded per the pre-registered
decision (reports/roi_loss_FINAL_DECISION.md), not part of the active
loss stack.

LR schedule fix vs. Stage A: Stage A used a cosine schedule sized for a
150-epoch budget but converged and early-stopped by epoch 39, so LR barely
moved (reports/STAGE_A_RESULTS.md). Stage B's convergence point is
genuinely unknown ahead of time - new data mix, warm-started weights - so
this uses ReduceLROnPlateau (mode=max on val PSNR) instead of guessing
another fixed horizon: it adapts to whatever the real convergence speed
turns out to be rather than assuming one.

Validation stays real-only (same leak-checked val split Stage A used) -
synthetic data has no real category/cluster ground truth to validate
against, and mixing it into validation would answer a different, weaker
question than "does this generalize to real SEM data." Per-cluster
reporting is repeated so clusters 11/14 (reports/phase2_deep_dive.md
Part 8) can be checked before/after the synthetic-data addition, as
...
def main():
    ...
    ap.add_argument("--synthetic-gt-dir", type=Path, default=Path("data/synthetic_external/GT"))
    ap.add_argument("--synthetic-noisy-dir", type=Path, default=Path("data/synthetic_external/NoisyLR"))
    ap.add_argument("--synthetic-manifest", type=Path, default=Path("data/synthetic_external/synthetic_manifest.csv"))
    ap.add_argument("--split-csv", type=Path,
                     default=Path("reports/phase2_source_clusters_stratified_leakchecked.csv"))
    ap.add_argument("--init-checkpoint", type=Path, default=Path("checkpoints/stage_a_best.pt"))
    ap.add_argument("--epochs", type=int, default=80)
    ap.add_argument("--patience", type=int, default=25, help="early-stop patience on val PSNR")
    ap.add_argument("--plateau-patience", type=int, default=5, help="ReduceLROnPlateau patience")
    ap.add_argument("--batch-size", type=int, default=16)
    ap.add_argument("--lr", type=float, default=5e-5, help="lower than Stage A's - fine-tuning, not training from scratch")
    ap.add_argument("--seed", type=int, default=123)
    ap.add_argument("--checkpoint-out", type=Path, default=Path("checkpoints/stage_b_best.pt"))
    ...
    print(f"Initialized from {args.init_checkpoint} (Stage A val_psnr={ckpt['val_psnr']:.3f})")
    ...
    final_val_df.to_csv(args.reports_dir / "stage_b_val_per_file_metrics.csv", index=False)
    per_cluster.to_csv(args.reports_dir / "stage_b_per_cluster_metrics.csv", index=False)
    pd.DataFrame(history).to_csv(args.reports_dir / "stage_b_training_history.csv", index=False)

    # Explicit before/after check for clusters 11 and 14 - the specific signal requested
    stage_a_per_cluster_path = args.reports_dir / "stage_a_per_cluster_metrics.csv"
    watch_clusters_comparison = {}
    if stage_a_per_cluster_path.exists():
        stage_a_pc = pd.read_csv(stage_a_per_cluster_path).set_index("cluster")
        stage_b_pc = per_cluster.set_index("cluster")
        for c in [11, 14]:
            if c in stage_a_pc.index and c in stage_b_pc.index:
                watch_clusters_comparison[str(c)] = {
                    "stage_a_psnr": float(stage_a_pc.loc[c, "mean_psnr"]),
                    "stage_b_psnr": float(stage_b_pc.loc[c, "mean_psnr"]),
                    "delta": float(stage_b_pc.loc[c, "mean_psnr"] - stage_a_pc.loc[c, "mean_psnr"]),
                }

    summary = {
        "n_train_real": len(train_files), "n_train_synthetic": len(synth_files),
        "n_train_total": len(train_files) + len(synth_files), "n_val": len(val_files),
        "init_checkpoint": str(args.init_checkpoint), "init_val_psnr": float(ckpt["val_psnr"]),
        "epochs_run": len(history), "epochs_budgeted": args.epochs,
        "best_epoch": best_epoch, "best_val_psnr": float(best_psnr),
        "best_val_ssim": float(ckpt_best["val_ssim"]),
        "psnr_gain_over_stage_a": float(best_psnr - ckpt["val_psnr"]),
        "n_clusters_in_val": int(per_cluster.shape[0]),
        "watch_clusters_11_14_comparison": watch_clusters_comparison,
        "worst_cluster_psnr": {
            "cluster": int(per_cluster.loc[per_cluster["mean_psnr"].idxmin(), "cluster"]),
            "mean_psnr": float(per_cluster["mean_psnr"].min()),
            "n": int(per_cluster.loc[per_cluster["mean_psnr"].idxmin(), "n"]),
        },
        "best_cluster_psnr": {
            "cluster": int(per_cluster.loc[per_cluster["mean_psnr"].idxmax(), "cluster"]),
            "mean_psnr": float(per_cluster["mean_psnr"].max()),
            "n": int(per_cluster.loc[per_cluster["mean_psnr"].idxmax(), "n"]),
        },
    ...
    with open(args.reports_dir / "stage_b_results.json", "w") as f:
