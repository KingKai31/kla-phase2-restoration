"""
Task 6: Stage A - the real (not diagnostic) training run on the full
4,785 real GT/NoisyLR pairs, using the leakage-checked stratified split
(reports/phase2_source_clusters_stratified_leakchecked.csv -
scripts/check_split_leakage.py found and fixed 4 cross-split
near-duplicate pairs before this run). Unlike
scripts/train_quick_checkpoint.py (25 epochs, diagnostic-only, built to
validate the confidence signal), this is the real Stage A model: trains
for a real epoch budget with best-checkpoint selection on val PSNR, and
reports per-cluster metrics (not just an aggregate number) since Part 1's
clustering already showed real, unignorable size imbalance (43.8x) across
this dataset - an aggregate metric alone could hide a cluster the model
handles poorly.

Same base 5-term StageBCompositeLoss as Stage B will use later for the
skip connection between stages to be meaningful, per this project's
established pattern of not editing a validated loss stack in place.
"""
import argparse
import json
import sys
...
def main():
    ...
    ap.add_argument("--checkpoint-out", type=Path, default=Path("checkpoints/stage_a_best.pt"))
    ...
    for epoch in range(args.epochs):
        ...
        history.append({"epoch": epoch, "val_psnr": float(val_psnr), "val_ssim": float(val_ssim),
                         "epoch_time_sec": epoch_time, "lr": scheduler.get_last_lr()[0]})
        print(f"epoch {epoch}: val_psnr={val_psnr:.3f} val_ssim={val_ssim:.4f} time={epoch_time:.1f}s")
    ...
    # Reload the best checkpoint and produce the final per-cluster report -
    # not the last epoch's model, since early stopping means the last epoch
    ...
    final_val_df.to_csv(args.reports_dir / "stage_a_val_per_file_metrics.csv", index=False)
    per_cluster.to_csv(args.reports_dir / "stage_a_per_cluster_metrics.csv", index=False)
    pd.DataFrame(history).to_csv(args.reports_dir / "stage_a_training_history.csv", index=False)
    ...
    summary = {
        ...
        "overall_val_psnr_recheck": float(final_val_df["psnr"].mean()),
        "overall_val_ssim_recheck": float(final_val_df["ssim"].mean()),
    ...
    with open(args.reports_dir / "stage_a_results.json", "w") as f:
