"""
Formal, re-runnable adversarial robustness suite for run.py - Phase 2
(SEM/NFFA-EUROPE data), ported from Phase 1's tests/test_run_py_robustness.py
(same 10 adversarial cases, same assertions) since run.py's guardrail
structure is architecturally identical this phase - only the checkpoint
and training data changed, not the robustness contract. Loads run.py
directly (matching how the real submission is invoked - no reimplementation
of its logic) and asserts specific, named guarantees against the hard
output spec: (H, W) shape, float32, values in [0, 1], no NaN/Inf, and no
single bad input crashing the whole batch.

Requires the real shipped checkpoint (models/checkpoint.pt, the Stage A
checkpoint - see reports/STAGE_A_RESULTS.md and
reports/SPECTRAL_FIX_ATTEMPT.md for why Stage A ships instead of a
Stage B variant) - this exercises the actual model forward pass, not a
mock, since that's the real robustness surface (e.g. the tiny-image case
below only fails via a real shape constraint inside NAFNetSR's padding,
which a mocked model would never surface).
...
def _build_adversarial_inputs(input_dir: Path) -> dict:
    """Same 10 cases as Phase 1's suite, seeded for reproducibility,
    returning {filename: original_array_or_None} (None for the corrupt
    file, which has no valid array)."""
...
def _assert_spec_compliant(arr: np.ndarray):
    """The universal hard-gate contract every output file must satisfy,
...
class TestValidButDegenerateInputs:
    ...
    def test_all_zero_input_does_not_break_fallback_sigma_estimation(self, adversarial_run):
        """all_zero is the case explicitly called out as a risk: skimage's
        estimate_sigma on a constant image could degenerate. Confirms it
        doesn't propagate into a bad output."""
...
class TestTinyImageModelPathLimit:
    """Same real, documented finding as Phase 1: NAFNetSR's forward pass
    reflect-pads each input up to a multiple of padder_size=16
    (2**len(enc_blk_nums)). PyTorch's reflect padding requires the pad
    amount to be strictly less than the input dimension, which fails for
    any side <=8px (pad needed >= side length). This IS caught by run.py's
    per-image try/except and correctly falls back to the classical path -
    the batch does not crash - but it means any image <=8px on either
    side silently downgrades to the much lower-quality classical
    restoration instead of the trained model. Same architecture, same
    limit, disclosed again rather than assumed fixed."""
...
class TestClassicalFallback:
    ...
    def test_nlm_denoising_actually_executes(self, run_module):
        """Regression test for the exact bug caught during THIS phase's
        final packaging pass (reports/run_py_compliance_checklist.md):
        PyWavelets was missing from the pod entirely - not just unpinned -
        which would have silently degraded every classical-fallback call
        to bicubic-only, exactly the Phase 1 bug recurring on new
        infrastructure. Fixed by installing PyWavelets and pinning it in
        both requirements.txt files. This test fails loudly if that
        regresses: it compares classical_fallback()'s real output against
        a bicubic-only reimplementation of the same upscale step and
        requires them to differ - if NLM silently stops running again, the
        two would go back to being identical and this test would catch it."""
