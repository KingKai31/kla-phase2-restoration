"""
Standalone inference script for KLA PS01 submission/benchmarking - Phase 2
(SEM/NFFA-EUROPE data). Built as run.py from the start (not renamed from a
dev/eval script later - a Phase 1 lesson carried forward directly).
...
(H, W) - no channel dimension - matching this dataset's own GT/NoisyLR .npy
convention exactly, same as Phase 1.

Self-contained by design, same Phase 1 lesson: the model architecture
(NAFNetSR and its building blocks, developed in src/models/nafnet.py in the
training repo) is inlined directly below rather than imported, so this
single file has no dependency on src/ or any other project code - only
pip-installed packages and the local models/checkpoint.pt. Verified
directly (not assumed) by the no-src-import audit in
reports/run_py_compliance_checklist.md.

Ships the Stage A checkpoint as the final model
(reports/STAGE_A_RESULTS.md, reports/STAGE_B_RESULTS.md,
reports/SPECTRAL_FIX_ATTEMPT.md document why: Stage B's synthetic-data
augmentation gave no measurable improvement, and one targeted, pre-
registered fix attempt correctly failed its own gate and was not pushed
through) - val PSNR=23.483dB, val SSIM=0.598 on the real leakage-checked
split.
...
local checkpoint file only.

Guardrails (never let one bad input crash the whole batch) - identical
structure to Phase 1's run.py, same rationale:
...
# in the training repo for design rationale; identical to Phase 1's, ported
# verbatim, no architecture changes this phase) -----------------------------
...
def sanitize_output(arr: np.ndarray) -> np.ndarray:
    ...
    produced `arr`. Does not trust any upstream clip/clamp - this re-clips,
    replaces any NaN/Inf directly, and asserts the result is finite,
    in-range, and (H, W) before returning. If any assertion here ever
    fires, that's a real upstream bug to fix, not something to paper over."""
...
def main():
    ...
    # location, not the process's current working directory - the same
    # cwd-independence fix Phase 1 found and fixed, applied from the start
